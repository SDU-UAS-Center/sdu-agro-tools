...
Color Distance Calculator Tutorial
==================================
...
You can notice that ``crop_from_orthomosaic.tif`` and ``crop_from_orthomosaic_annotated.tif`` are identical images, except for the annotations (red blobs) on some of the pumpkins.
...
Additionally we can compute the color distribution using a :ref:`shape file <calculate-color-distribution-shape>` instead of images; change the :ref:`tiles dimension <change-tiles-dimension>` involved in the :ref:`multi-tiles simultaneous processing <multi-tiles-processing>`, and :ref:`save tiles image <save-tiles-images>` as a different result.
