from PyQt5.QtCore import QRunnable, pyqtSignal, QObject, QThreadPool, QEventLoop,  QMutex
import os
import numpy as np
import rasterio
...
class MultiTilesWorker(QRunnable):
    ...
    def run(self):
        try:
            ...
            result = self.func(self.tile)  # Run the task
...
class ColorBasedSegmenter(QObject):
    ...
    def __init__(self, tiles_list, colormodel, param):
        ...
        self.task_queu = list(self.tiles_list) # Copylist of tiles
    ...
    def handle_result(self, tile, result):
        ...
        if self.completed_tasks > 4:#len(self.tiles_list):
    ...
    def apply_colormodel_multi_tiles(self):
        ...
        for tile in self.tiles_list:
            ...
            worker.signals.finished.connect(lambda: print(f"Thread for tile {tile.tile_number} finished"))
            thread_pool.start(worker)
            active_threads += 1
            # tile, result = self.process_tile(tile)
            # self.handle_result(tile, result)
            cont+=1
            if active_threads > 4: 
